#ifndef MEMCACHEPP_MEMCACHE_DETAIL_DIRECTIVES_ADD_HPP_
#define MEMCACHEPP_MEMCACHE_DETAIL_DIRECTIVES_ADD_HPP_
...
namespace memcache { namespace detail {
    ...
    template <class T>
        struct add_directive {
            
            explicit add_directive(std::string const & key, T const & value, boost::uint16_t flags, time_t timeout, time_t failover_timeout) :
